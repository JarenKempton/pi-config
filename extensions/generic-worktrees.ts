...
function isSalesAiRepo(cwd: string): boolean {
  try {
    return path.basename(mainRepoRoot(cwd)) === "salesai";
  } catch {
    return false;
  }
}
...
async function createGenericWorktree(ctx: any, input: string) {
...
async function deleteWorktree(ctx: any, selected: Worktree | string) {
  ...
  ctx.ui.setStatus?.("worktrees", `Deleting ${path.basename(wt.path)}…`);
  ctx.ui.notify(`Checking worktree status:\n${wt.path}`, "info");
  ...
  ctx.ui.notify(`Removing worktree:\n${wt.path}`, "info");
  ...
  ctx.ui.notify("Pruning stale worktree metadata…", "info");
  ...
  if (wt.branch && wt.branch !== "(detached)" && wt.branch !== DEFAULT_BASE_BRANCH) {
    ...
    if (deleteBranch) {
      ctx.ui.notify(`Deleting local branch ${wt.branch}…`, "info");
...
export default function genericWorktrees(pi: ExtensionAPI) {
  async function createWorktree(ctx: any, input: string) {
    ...
    await createGenericWorktree(ctx, input);
  }
