...
- For browser QA, UI verification, web bug reproduction, authenticated dashboards, or end-to-end UI checks, automatically use the `browser_qa` tool. It controls the user's existing visible browser through Codex Computer Use and is the reliable default; do not ask the user to repeatedly authorize Playwright/CDP. Use the `test-in-browser` skill and `authenticated-browser` MCP only when DOM, console, network, or trace inspection is specifically required and its health preflight succeeds. If MCP attachment fails or times out, fall back to `browser_qa` immediately. Reuse the running browser; do not launch a separate headless session. Treat login/SSO/MFA screens as a user authentication handoff, then resume the same browser loop.
