...
- Tightened the footer to two dashboard rows: repository details sit beside the cwd, costs use two decimals, and throughput/window-size noise is removed. Quotas use clear provider labels with percentage and reset countdown. Stale Claude data remains visible as a muted, explicitly stale percentage summary without misleading reset times; expired current data and weekly-only Codex model buckets stay hidden unless a canonical 5-hour Codex window returns.
