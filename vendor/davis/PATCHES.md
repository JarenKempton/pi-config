...
- Tightened the footer to two dashboard rows: repository details sit beside the cwd, costs use two decimals, and throughput/window-size noise is removed. Quotas use clear provider labels with percentage and reset countdown; stale or expired data stays hidden, as do weekly-only Codex model buckets unless a canonical 5-hour Codex window returns.
