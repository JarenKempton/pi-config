...
- Tightened the footer to two dashboard rows: repository details sit beside the cwd, costs use two decimals, throughput/window-size noise is removed, and icon-led quota summaries include percentage plus reset countdown. Weekly-only Codex model buckets are hidden unless a canonical 5-hour Codex window returns.
