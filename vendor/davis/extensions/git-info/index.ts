...
export default function gitInfo(pi: ExtensionAPI) {
  ...
  pi.registerCommand("lg", {
    description: "Open Hunk's live diff review in the exact current worktree",
    handler: async (_args, ctx) => {
      const hunk = fsExists("/opt/homebrew/bin/hunk")
        ? "/opt/homebrew/bin/hunk"
        : "hunk";
      if (process.platform !== "darwin") {
        ctx.ui.notify(
          `Run in another terminal from this worktree: ${hunk} diff --watch`,
          "warning",
        );
        return;
      }

      const command = `cd ${shellQuote(ctx.cwd)} && exec ${shellQuote(hunk)} diff --watch`;
      ...
      child.on("error", (error) => {
        ctx.ui.notify(`Failed to open Hunk: ${error.message}`, "error");
      });
      child.on("close", (code) => {
        ctx.ui.notify(
          code === 0
            ? `Opened Hunk for ${ctx.cwd}`
            : `Failed to open Hunk: ${stderr.trim() || `osascript exited ${code}`}`,
          code === 0 ? "info" : "error",
        );
      });
    },
