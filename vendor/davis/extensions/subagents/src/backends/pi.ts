...
} from "../../../shared/project-tool-policy.ts";
...
const makePiSession = (
  ...
  Effect.gen(function* () {
    ...
    const session = yield* Effect.tryPromise({
      try: async () => {
        ...
        const { session } = await createAgentSession({
          ...
          model,
          thinkingLevel,
          ...childToolPolicy(task.profile ?? "scout"),
        });
    ...
    const projectWriteGuard = createProjectWriteGuard(task.cwd);
    const applyToolGuards = () => {
      if ((task.profile ?? "scout") === "worker") {
        projectWriteGuard.apply(session);
      }
