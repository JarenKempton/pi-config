...
const makeCodexSession = (
  ...
  Effect.gen(function* () {
    ...
    const child = yield* Effect.try({
      try: () =>
        spawn(binary, codexServerArgsForProfile(task.profile ?? "scout"), {
          cwd: task.cwd,
          env: process.env,
    ...
    const threadResult = yield* Effect.tryPromise({
      try: async () => {
        ...
        // Headless children cannot answer approval prompts; profiles map to
        // non-interactive Codex approval/sandbox settings. Only confirmed
        // unrestricted spawns receive danger-full-access.
        return request("thread/start", {
          cwd: task.cwd,
          ...codexOptionsForProfile(task.profile ?? "scout"),
          ephemeral: false,
