...
const makeClaudeSession = (
  ...
  Effect.gen(function* () {
    ...
    const profileOptions = claudeOptionsForProfile(profile);
    const nativeQuery = yield* Effect.try({
      try: () =>
        query({
          ...
          options: {
            cwd: task.cwd,
            // Headless children cannot answer approval prompts. Profiles map
            // to non-interactive permission policies; only user-confirmed
            // unrestricted spawns use the previous bypass behavior.
            ...
            disallowedTools: [
              ...new Set([...((profileOptions as any).disallowedTools ?? []), "Agent", "Task"]),
            ],
            // Restrictive profiles ignore filesystem settings entirely so a
            // user/project allow rule cannot widen their unattended boundary.
            // A confirmed unrestricted child keeps normal trusted-project
            // behavior; an untrusted project still gets user settings only.
    ...
    const handleResult = (result: SDKResultMessage) => {
      ...
      state.costUsd = Math.max(state.costUsd, result.total_cost_usd);
      emit({
        _tag: "UsageChanged",
