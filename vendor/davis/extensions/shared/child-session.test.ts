...
test("default child policy is a read-only allowlist with explicit extras", async () => {
  await withTempDir(async (directory) => {
    ...
    const { session } = await createAgentSession({
      ...
      ...childToolPolicy("scout", ["structured_output"]),
    });
    ...
    assert.equal(allTools.has("fixture_extension_tool"), false);
    assert.equal(activeTools.has("fixture_extension_tool"), false);
    ...
    assert.equal(activeTools.has("read"), true, "read should stay active");
    for (const builtin of ["bash", "edit", "write"]) {
      assert.equal(
        activeTools.has(builtin),
        false,
        `${builtin} should be inactive by default`,
      );
    }
