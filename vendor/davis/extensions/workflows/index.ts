...
type WorkflowProfile = "scout" | "researcher" | "worker" | "unrestricted";
const WORKFLOW_PROFILES = ["scout", "researcher", "worker", "unrestricted"] as const;
...
const WorkflowParams = Type.Object({
  ...
  profile: Type.Optional(
    Type.Union([
      ...
      description:
        "Default safety profile for workflow agent() calls: scout/researcher are read-only, worker is project-confined with shell denied unless sandboxed, unrestricted requires foreground UI confirmation.",
    }),
  ),
});
...
export default function workflows(pi: ExtensionAPI) {
  pi.on("session_start", () => {
    cleanupOldWorkflowArtifacts(path.join(getAgentDir(), "workflows"));
  });
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, signal, onUpdate, ctx) {
      ...
      const defaultProfile = ((WORKFLOW_PROFILES as readonly string[]).includes(params.profile ?? "")
        ? params.profile
        : "scout") as WorkflowProfile;
      if (defaultProfile === "unrestricted") {
        if (background || !ctx.hasUI) {
          throw new Error("Workflow profile=\"unrestricted\" requires foreground UI confirmation and is unavailable for background/headless runs.");
        }
        const ok = await ctx.ui.confirm(
          "Allow unrestricted workflow?",
          `Workflow ${meta.name ?? runId} may launch unrestricted child agents in ${ctx.cwd}.`,
        );
        if (!ok) throw new Error("Unrestricted workflow was not confirmed by the user.");
      }
      ...
      const agentFn = async (
        ...
        const label =
          typeof opts.label === "string" && opts.label.trim()
            ...
            : `agent-${index}`;
        const profile = ((WORKFLOW_PROFILES as readonly string[]).includes(String(opts.profile ?? ""))
          ? String(opts.profile)
          : defaultProfile) as WorkflowProfile;
        if (profile === "unrestricted") {
          if (background || !ctx.hasUI) {
            return {
              ok: false,
              output: "",
              error: `agent "${label}": profile=\"unrestricted\" requires foreground UI confirmation`,
            };
          }
          const ok = await ctx.ui.confirm(
            "Allow unrestricted workflow agent?",
            `Agent ${label} may run with unrestricted permissions in ${ctx.cwd}.`,
          );
          if (!ok) {
            return {
              ok: false,
              output: "",
              error: `agent "${label}": unrestricted profile was not confirmed`,
            };
          }
        }
