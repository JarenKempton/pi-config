...
const CLEANUP_MAX_REMOVALS = 200;
...
export function cleanupOldWorkflowArtifacts(baseDir: string, now = Date.now()) {
  let removed = 0;
  let names: string[] = [];
  try {
  ...
  for (const name of names.sort()) {
    if (removed >= CLEANUP_MAX_REMOVALS) break;
    const runDir = path.join(baseDir, name);
    try {
      const stat = fs.statSync(runDir);
      if (!stat.isDirectory()) continue;
      if (now - stat.mtimeMs <= ARTIFACT_RETENTION_MS) continue;
      fs.rmSync(runDir, { recursive: true, force: true });
      removed += 1;
    } catch {
      // Best-effort bounded startup cleanup.
    }
  }
  return removed;
}
