...
import { createProjectWriteGuard } from "../shared/project-tool-policy.ts";
...
export function guardWorkflowChildTools(
  ...
  timeoutMs?: number,
  options: { profile?: ChildSafetyProfile; cwd?: string } = {},
) {
  const timeoutGuard = createToolCallTimeoutGuard(timeoutMs);
  const writeGuard = options.cwd
    ? createProjectWriteGuard(options.cwd)
    : undefined;
  const apply = () => {
    if (options.profile === "worker") writeGuard?.apply(session);
...
export async function runAgent(
  ...
  try {
    ...
    unsubscribeToolTimeout = guardWorkflowChildTools(
      ...
      { profile: options.profile ?? "scout", cwd: options.cwd },
    );
  } catch (error) {
