...
- `/usage` (or `Alt+U`) opens refreshable Codex, Claude, and Cursor account status. Codex/Claude quota windows use progress bars; Cursor shows the authenticated subscription tier and active model because its CLI does not expose deterministic quota percentages. It is safe during an active run and never writes to the transcript or model context. The footer shows current quota percentages and reset countdowns with clear provider labels; stale or expired data stays hidden, as do weekly-only Codex model buckets unless a canonical 5-hour Codex window is available.
