...
- Settled tool executions collapse to one transcript line while live tools retain their normal detail. The compact row keeps the tool, target/command, outcome, and result size; `Ctrl+O` restores the full blocks whenever details are needed. Reasoning and compaction summaries follow the same dense-history approach.
