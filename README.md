...
- Reasoning traces and tool blocks remain fully visible throughout the complete agent run so active work can be inspected and steered across internal model/tool turns. Only after `agent_settled` does dense history collapse reasoning and hide settled tool blocks, leaving the transcript focused on user prompts and final answers. `Ctrl+T` restores reasoning and `Ctrl+O` restores tool blocks whenever details are needed; compaction summaries remain condensed.
...
- `/usage` (or `Alt+U`) opens refreshable Codex, Claude, and Cursor account status. Opening or refreshing the overlay uses Claude's structured SDK usage control without a model turn or token cost, then atomically updates the private cache; every Claude subagent result refreshes that same cache through its existing SDK session. Experimental API failures retain the prior cache. Codex/Claude quota windows use progress bars; Cursor shows the authenticated subscription tier and active model because its CLI does not expose deterministic quota percentages. It is safe during an active run and never writes to the transcript or model context. The footer stacks providers vertically to preserve horizontal space, uses the open left side of secondary provider rows for extension statuses such as MCP, and applies the same green/warning/error thresholds to every context and quota percentage. Stale Claude data remains visible as a muted, explicitly stale percentage summary without misleading reset times; expired current data and model-specific Codex buckets stay hidden. Each canonical Codex window is shown independently, so weekly usage remains visible even when no 5-hour window exists.
- `/cost [today|7d|30d|all]` (or `Alt+C`) opens local API-equivalent history in a period-switchable overlay with model totals and proportional cost bars. It refreshes model rates from the official OpenAI and Anthropic pricing Markdown every six hours, caches the catalog locally, and falls back explicitly when a source is unavailable. The footer separately shows two-decimal current-chat cost, current-session subagent cost, and today's cross-model local tally.
