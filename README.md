...
- `/cost [today|7d|30d|all]` (or `Alt+C`) opens local API-equivalent history in a period-switchable overlay with model totals and proportional cost bars. The footer separately shows two-decimal current-chat cost, current-session subagent cost, and today's cross-model local tally.
