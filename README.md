...
- Settled tool executions collapse to one transcript line while live tools retain their normal detail. The compact row uses an aligned, low-noise ledger layout with the tool and target/command on the left and outcome, result size, and `⌃O` expansion hint on the right; `Ctrl+O` restores the full blocks whenever details are needed. Reasoning and compaction summaries follow the same dense-history approach.
